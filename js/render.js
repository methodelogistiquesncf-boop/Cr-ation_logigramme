...
// === NOUVEAU: Calcul automatique de la taille ===
...
const width = Math.max(maxLineWidth + 40, 100);
const height = Math.max(lines.length * lineHeight + 30, 50);
...
if (node.autoResize === false) return;
...
// === NOUVEAU: Poignées de redimensionnement ===
const handles = document.createElement('div');
handles.className = 'resize-handles';
['n','s','e','w','ne','se','sw','nw'].forEach(pos => {
...
if (pos.includes('e')) node.width = Math.max(80, startW + dx);
if (pos.includes('s')) node.height = Math.max(40, startH + dy);
if (pos.includes('w')) {
node.width = Math.max(80, startW - dx);
node.x = startLeft + (startW - node.width);
}
if (pos.includes('n')) {
node.height = Math.max(40, startH - dy);
node.y = startTop + (startH - node.height);
}
